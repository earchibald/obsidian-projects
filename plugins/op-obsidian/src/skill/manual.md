...
| `op-set-tasks` | `issue`, `body` | `append=true` | replace (or, with `append=true`, extend) the issue body's `## Tasks` checklist. `body` is plain markdown — typically `- [ ] <ID>.<N> — <title>` lines, with completed entries marked `- [completed]` or `- [x]`. Frontmatter, the `# Title`, and every other H2 section are preserved; payload must not contain bare `## ` H2 headings (would terminate the section). Use this in place of raw `Edit`/`Write` on the issue body when reconciling the Tasks list as work proceeds. |
| `op-task-create` | `issue`, `title` | `taskId`, `body`, `status` | create a new TASK note under `Projects/<slug>/TASKS/`. Auto-allocates `<issue>.<N>` from the existing children (lowest unused N starting at 1), or honors an explicit `taskId=<issue>.<N>` from the caller. Frontmatter carries `op_managed: true` and `issueLink: "[[<basename>]]"` — the latter is what `op-resolve` walks to trash linked tasks atomically. Also writes the auto-managed flag the Phase 2 pretool guard will look for. |
| `op-task-set-status` | `taskId`, `status` | — | flip the `status:` frontmatter on a TASK note. `status` ∈ `pending|in-progress|completed|blocked`. Use this when you'd otherwise `Edit` a TASK note's frontmatter; the plugin's atomic `processFrontMatter` write preserves every other YAML key (including the `op_managed: true` flag). |
| `op-task-append-note` | `taskId`, `body` | — | append a markdown block to a TASK note's body, glued onto the existing body with a blank-line separator. Use to record progress / sub-step output without reaching for raw `Edit`. Frontmatter is left untouched. |
| `op-append-note` | `issue`, `body` | — | append a markdown block to an issue's `## Notes` section, creating the section if it doesn't exist. A fast, two-arg shorthand for `op-set-section issue=<id> name=Notes content=<body> append=true` — designed for agents to log actions as they happen rather than storing them up. Frontmatter and every other H2 section are preserved. For task notes, use `op-task-append-note` instead. |
| `op-doc-create` | `project`, `doc_type`, `title` | `body` | create a vault-only DOC note under `Projects/<slug>/DOCS/`. `doc_type` ∈ `plan|spec|adr|runbook` and is mirrored into both frontmatter and a `doc/<doc_type>` tag. Refuses paths under `DOCS/superpowers/` — that tree is the repo-symlinked superpowers docs and is governed by the project repo's normal git workflow (not vault discipline). |
| `op-doc-edit` | `path`, `body` | `section`, `append=true` | edit a vault-only DOC note. With `section=<H2>` the rewrite is scoped to that section (same fence-aware splitter as `op-set-section`); without `section` the body is appended in full (same idiom as `op-task-append-note`). Refuses paths under `DOCS/superpowers/`. The DOC's frontmatter and other H2 sections are preserved. |
| `op-flush-vault-history` | `issue` | `subject` | **destructive — rewrites HEAD via `git reset --soft`.** Squashes the consecutive run of vault-git commits at HEAD whose subject references the issue id into a single commit. Companion to the opt-in `vaultGit.autoCommit` setting (default off): per-call commits multiply rapidly during a long issue, this collapses them into one canonical commit at resolve time. Refuses cleanly when the vault is not a git repo, when HEAD doesn't reference the issue id, or when the run has fewer than 2 commits to squash. |
...
7. **Mirror every TASK note into a `## Tasks` checklist in the issue body.** After creating the TASK notes (planned upfront, or fix-up tasks discovered mid-session), append a line to the issue body's `## Tasks` section for each one:
   ...
   When a TASK note flips to `status: completed`, also **append a `### <ISSUE-ID>.<N> — <title>` block under `## Notes`** recording what was done and any deviations from the plan. Use `obsidian vault=<name> op-append-note issue=<ISSUE-ID> body="### <ISSUE-ID>.<N> — <title>\n\n…"` — a fast two-arg shorthand for `op-set-section` with `name=Notes` and `append=true` built in. Log progress as it happens rather than batching notes up for a later summary. Idempotent at the agent level: before appending, check the current Notes section (`obsidian vault=<name> read path="…"`); if a block with the same `### <ISSUE-ID>.<N>` heading already exists, call `obsidian vault=<name> op-set-section issue=<ISSUE-ID> name=Notes content="…"` (no `append=true`) with the merged Notes content to update in place rather than duplicating.
