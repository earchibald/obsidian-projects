...
**Phase 3 guard layer (OP-260, default off).** Setting `agentDiscipline.newFileGuard` (Settings → op-obsidian → Worktree enforcement → "Refuse new files under ISSUES / RESOLVED ISSUES / TASKS") gates a third layer in the same `pretool-worktree-guard.sh`. When on, the script checks whether the `file_path` is a **new file** (does not yet exist on disk) whose path matches `*/Projects/<slug>/{ISSUES,RESOLVED ISSUES,TASKS}/*` and exits 2 with a hint toward the right creation endpoint (`op-new` for issues, `op-task-create` for TASK notes). Existing-file edits at those paths fall through to Phase 2 (managed-note check). Per-call escape hatch: `OP_ALLOW_NEW_FILE=1`. Layers 2 and 3 share a single stdin parse of the PreToolUse JSON payload — when both are enabled the script reads stdin once and applies both checks in sequence; each has its own independent override env var. Phase 6 of OP-218 will flip the default to on after soak.
