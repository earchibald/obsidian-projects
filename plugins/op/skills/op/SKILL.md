...
| `op-work` | `issue` | `agent`, `agent_session`, `force` | sets `status: in-progress`; creates the initial TASKS note. When `agent=` (and optionally `agent_session=`) are passed, also writes `agent:` / `agent_session:` to the issue and reports a `conflict` in the JSON payload if a different agent or session was already registered (override with `force=true`). |
