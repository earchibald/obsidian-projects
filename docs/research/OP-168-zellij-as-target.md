...
| **Total non-generated source** | **~2.7k** | |
...
| **(c) both** | Everything in (a) and (b) | nothing | ~2.7k LoC source + ~19k generated removed; ~400 LoC added | **Maybe later.** Cleanest architecture *if and when* the multi-client mirror gap closes and we accept the OSC 2 + iTerm-CC losses. |
...
- **~2.7k LoC of source deleted** (orchestrator + iterm/* + tmux observability glue), plus ~19k LoC of generated protobuf types — assuming role (c).
...
The mirror gap is structural. The current per-pane-viewer pattern is genuinely useful and not trivially replaceable. Pre-1.0 software with a brand-new automation surface is the wrong base for a multi-week migration that, if it ships, deletes ~2.7k LoC and replaces three integration points at once.
